...
/**
 * \fn int sun7i_mbr_check(void *_mbr, int test)
 ...
 */
int sun7i_mbr_check(void *_mbr, int test)
{
    SUN7I_MBR *mbr = (SUN7I_MBR *)_mbr;

    switch(test) {
    case MBR_CHECK_MAGIC:
        if(!strncmp((char *)mbr -> magic, SUN7I_MBR_MAGIC, strlen(SUN7I_MBR_MAGIC)))
            return(1);
        break;
    case MBR_CHECK_VERSION:
        if(mbr -> version == SUN7I_MBR_VERSION)
            return(1);
        break;
    case MBR_CHECK_CRC32:
        if(*(unsigned int *)mbr == calc_crc32(mbr, SUN7I_MBR_SIZE))
            return(1);
        break;
    }
    return(0);
...
/**
 * \fn void *sun7i_mbr_get(void *_mbr, int info)
 ...
 */
void *sun7i_mbr_get(void *_mbr, int info)
{
    SUN7I_MBR *mbr = (SUN7I_MBR *)_mbr;
...
/**
 * \fn void sun7i_mbr_set(void *_mbr, int info, void *data)
 ...
 * \return None..
 */
void sun7i_mbr_set(void *_mbr, int info, void *data)
{
    SUN7I_MBR *mbr = (SUN7I_MBR *)_mbr;
    ...
    case SUNXI_MBR_VERSION:
        ...
        break;
    case SUNXI_MBR_MAGIC:
        strncpy((char *)mbr -> magic, data, SUN7I_MBR_MAGiC_LEN);
        break;
    case SUNXI_MBR_COPY:
    ...
    case SUNXI_MBR_PARTCOUNT:
        ...
        break;
    case SUNXI_MBR_RESERVED:
        memcpy(mbr -> res, data, sizeof(char) * SUN7I_MBR_RESERVED);
        break;
    case SUNXI_MBR_STAMP:
        memcpy(mbr -> stamp, data, sizeof(int));
        break;
    }
    mbr -> crc32 = calc_crc32(mbr, SUN7I_MBR_SIZE);
}

/**
 * \fn void *sun7i_part_get(void *_mbr, int partition, int info)
 ...
 * \return a void pointer to the requested information.
 */
void *sun7i_part_get(void *_mbr, int partition, int info)
{
    SUN7I_MBR *mbr = (SUN7I_MBR *)_mbr;

    if((intptr_t)partition < SUN7I_MBR_PART_COUNT) {
        switch(info) {
        ...
        case SUNXI_PART_USERTYPE:
            ...
            break;
        case SUNXI_PART_KEYDATA:
            return(&mbr -> array[partition].keydata);
            break;
        case SUNXI_PART_RO:
...
/**
 * \fn void sun7i_part_set(void *_mbr, int partition, int info, void *data)
 ...
 * \return None..
 */
void sun7i_part_set(void *_mbr, int partition, int info, void *data)
{
    SUN7I_MBR *mbr = (SUN7I_MBR *)_mbr;
    ...
    if((intptr_t)partition < SUN7I_MBR_PART_COUNT) {
        ...
        case SUNXI_PART_LENLO:
            ...
            break;
        case SUNXI_PART_CLASSNAME:
            strncpy((char *)mbr -> array[partition].classname, data, SUN7I_MBR_NAME_LEN);
            break;
        case SUNXI_PART_NAME:
            strncpy((char *)mbr -> array[partition].name, data, SUN7I_MBR_NAME_LEN);
            break;
        case SUNXI_PART_USERTYPE:
            mbr -> array[partition].user_type = (intptr_t) data;
            break;
        case SUNXI_PART_KEYDATA:
            mbr -> array[partition].keydata = (intptr_t) data;
            break;
        case SUNXI_PART_RO:
            mbr -> array[partition].ro = (intptr_t) data;
            break;
        case SUNXI_PART_RESERVED:
            memcpy(mbr -> array[partition].res, data, sizeof(char) * SUN7I_MBR_PART_RES);
            break;
        }
    }
    mbr -> crc32 = calc_crc32(mbr, SUN7I_MBR_SIZE);
...
/**
 * \fn void sun7i_part_copy(void *_mbr, int dst, int src)
...
{
    SUN7I_MBR *mbr = (SUN7I_MBR *)_mbr;
    if( ((intptr_t)dst < SUN7I_MBR_PART_COUNT) && ((intptr_t)src < SUN7I_MBR_PART_COUNT)  ) {
        ...
        memcpy(mbr -> array[dst].classname, mbr -> array[src].classname, sizeof(char) * SUN7I_MBR_NAME_LEN);
        memcpy(mbr -> array[dst].name, mbr -> array[src].name, sizeof(char) * SUN7I_MBR_NAME_LEN);
        mbr -> array[dst].user_type = mbr -> array[src].user_type;
        mbr -> array[dst].keydata = mbr -> array[src].keydata;
        mbr -> array[dst].ro = mbr -> array[src].ro;
        memcpy(mbr -> array[dst].res, mbr -> array[src].res, sizeof(char) * SUN7I_MBR_PART_RES);
        mbr -> crc32 = calc_crc32(mbr, SUN7I_MBR_SIZE);
    }
}
