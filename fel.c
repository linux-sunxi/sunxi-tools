		    ...
		    uint32_t *dst, size_t count)
{
	...
	aw_fel_read(usb, sram_info->scratch_addr + LCODE_ARM_SIZE,
...
int main(int argc, char **argv)
{
	...
	if (argc <= 1) {
		...
		printf("Usage: %s [options] command arguments... [command...]\n"
			...
			"	copyl dest source count		Copy 32-bit values within device memory\n"
	...
	while (argc > 1 ) {
		...
		if (strncmp(argv[1], "hex", 3) == 0 && argc > 3) {
			...
			skip = 3;
		} else if (strcmp(argv[1], "copyl") == 0 && argc > 4) {
			/* three parameters: destination addr, source addr, word count */
			aw_fel_copyl_n(handle, strtoul(argv[2], NULL, 0),
