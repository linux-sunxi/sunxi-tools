...
static long ioctl_aw(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case AWUSB_IOCRESET:
        printk("ioctl_aw--AWUSB_IOCRESET\n");
		break;

	case AWUSB_IOCSET:
        printk("ioctl_aw--AWUSB_IOCSET\n");
        
        if (copy_from_user(&param_tmp, (void __user *)arg, sizeof(param_tmp)))
		    retval= -EFAULT;

        printk("param_tmp.test_num = %d\n", param_tmp.test_num);
        printk("param_tmp.p1 = %d\n", param_tmp.p1);
        printk("param_tmp.p2 = %d\n", param_tmp.p2);
        printk("param_tmp.p3 = %d\n", param_tmp.p3);
                    
		break;
        
   case AWUSB_IOCGET:
        printk("ioctl_aw--AWUSB_IOCGET\n"); 
    ...
    case AWUSB_IOCSEND:
        ...
        if (!(buffer = kmalloc(buffer_len, GFP_KERNEL))) {
            err("AWUSB_IOCSEND: Not enough memory for the send buffer!");
            retval = -ENOMEM;
            goto err_out;
        }
	...
	for(ii = 0; ii < buffer_len; ii ++)
        ...
        printk("ioctl_aw--AWUSB_IOCSEND-exit\n"); 
        break;

    case AWUSB_IOCRECV:
        printk("ioctl_aw--AWUSB_IOCRECV\n"); 
        ...
        printk("value=%d\n", value);
        if (!(buffer = kmalloc(buffer_len, GFP_KERNEL))) {
            err("AWUSB_IOCSEND: Not enough memory for the receive buffer!");
            retval = -ENOMEM;
            goto err_out;
        }
    ...
    case AWUSB_IOCSEND_RECV:
        printk("ioctl_aw--AWUSB_IOCSEND_RECV\n");
