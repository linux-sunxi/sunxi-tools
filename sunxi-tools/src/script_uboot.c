...
/*
 */
void out_u32_member(FILE *out, const char *key, int hexa,
...
void out_gpio_member(FILE *out, const char *key,
...
void out_null_member(FILE *out, const char *key) {
    fprintf(out, "\t/* %s is NULL */\n", key);
}

int out_member(FILE *out, const char *key, int mode,
...
static int generate_dram_struct(FILE *out, struct script_section *sp) {
    ...
    for (mp = dram_members; mp < dram_members + ARRAY_SIZE(dram_members); mp++) {
...
static int generate_pmu_struct(FILE *out, struct script_section *target,
    ...
    for (mp = pmu_members; mp < pmu_members + ARRAY_SIZE(pmu_members); mp++) {
